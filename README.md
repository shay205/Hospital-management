...
For Render, set `DJANGO_ALLOWED_HOSTS` to `hospital-management-bkaa.onrender.com` and set `CORS_ALLOWED_ORIGINS` to the exact origin serving the frontend, separated by commas. For the local frontend server, use `http://localhost:5500,http://127.0.0.1:5500`. After changing Render environment variables or deploying this settings change, redeploy the service so Django reloads the configuration.
